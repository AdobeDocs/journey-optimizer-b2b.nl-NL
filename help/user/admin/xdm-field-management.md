...
De gebieden van het Gegevensmodel van de ervaring (XDM) zijn schemaelementen die gegevens aan de [!DNL Journey Optimizer B2B Edition] toepassing verstrekken. Gebruik XDM-velden als filters en beperkingen in reisknooppunten, inkoopgroepen en voor inhoudsfuncties, zoals personalisatie van e-mail en voorwaardelijke inhoud.
...
Adobe Experience Platform (AEP)-schema&#39;s bevatten gewoonlijk veel velden in complexe hiërarchieën. Het doorlopen van XDM-schemabomen kost tijd. Het XDM gebiedbeheer stroomlijnt gebiedsselectie door slechts de gebieden te tonen die voor uw reizen, het kopen groepen, en verpersoonlijking relevant zijn.  Beheerders schakelen deze velden in voor gebruik in Journey Optimizer B2B edition, inclusief de velden die alleen-lezen of bewerkbaar zijn.
...
>[!NOTE]
>
>Een gebied moet eerst _Beheerd_ zijn alvorens het _Updatable_ kan zijn. De _Updatable gebieden_ die u selecteert moeten in uw user-provided schema bestaan. Uw schema bevat mogelijk geen vereiste velden, behalve voor door het systeem gedefinieerde velden.
...
>[!IMPORTANT]
>
>Hulplijnen voor velden die kunnen worden bijgewerkt:
>
>* Schema&#39;s - Voor de klasse Individueel profiel XDM moeten alle vereiste velden in het schema door het systeem worden gedefinieerd, zoals `identityMap` of `personID` .
>* Datasets - Gebruik geen gegevensset die al voor een ander doel wordt gebruikt. Als beste praktijken, creeer specifieke datasets specifiek voor het opslaan van updatable gebieden. Gebruik een afzonderlijke dataset voor elke klasse XDM.
...
Met relationele schema&#39;s kunt u aangepaste gegevensklassen maken. Met toegang tot veelvoudige datasets, kunt u klassen tot stand brengen die specifiek aan uw gegevensbehoeften worden aangepast. Gebruik relationele schema&#39;s voor bedrijfsentiteiten zoals aankopen, licenties en registraties van gebeurtenissen bij reisbeslissingen en e-mailpersonalisatie. U kunt maximaal 20 schema&#39;s en maximaal 50 velden per schema selecteren.

Er zijn veelvoudige eigenschappen die het gebruik van de gevormde relationele schema&#39;s en gebieden steunen:

* [Inhoud aanpassen](../content/personalization.md#custom-datasets)
* [Reisbeslissingen (gesplitste paden)](../journeys/split-merge-paths-nodes.md#custom-data-filtering)
* [&#x200B; het Kopen groepsrollen &#x200B;](../buying-groups/buying-groups-role-templates.md#add-the-template-roles) (B2B Persoon slechts)
...
>[!NOTE]
>
>Deze functie biedt momenteel ondersteuning voor gebruiksgevallen van aan accounts gerelateerde en aan personen gerelateerde aangepaste objecten, met plannen om in de toekomst meer gebruiksgevallen voor objecten buiten de box te ondersteunen.
...
1. Klik op **[!UICONTROL Select relational XDM schema]** .
   ...
   >[!NOTE]
   >
   >In deze bètaeigenschapversie, slechts worden de _Rekening en de Mensen vele-aan-één Voorwerpen van de Douane_ gesteund.
