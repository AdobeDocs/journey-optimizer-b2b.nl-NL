---
title: Instellen voor het bijhouden en verzenden van e-mail
...
# Instellen voor het bijhouden en verzenden van e-mail

Adobe Journey Optimizer B2B edition gebruikt de functies van het e-mailkanaal en het bijhouden van gebeurtenissen in het bijgevoegde Marketo Engage-exemplaar. Om ervoor te zorgen dat de e-maillevering zoals verwacht voor organisaties werkt die beperkende firewall of volmachtsservermontages gebruiken, moet een systeembeheerder bepaalde domeinen en IP adreswaaiers aan de lijst van gewenste personen toevoegen.

>[!NOTE]
>
>Als uw organisatie al gebruikmaakt van het aangesloten Marketo Engage-exemplaar om uw marketingbewerkingen uit te voeren, moeten deze protocollen en configuraties al zijn geïnstalleerd.
...
1. [DNS-records maken voor bestemmingspagina&#39;s en e-mail](#create-dns-records-for-landing-pages-and-email)
...
>[!NOTE]
>
>E-mailleveringsservices en advies zijn afzonderlijke betaalde aanbiedingen van Adobe. Als u ondersteuning nodig hebt of wilt van het leveringsteam voor uw Journey Optimizer B2B edition-instantie, moet u voor die instantie een van de pakketten Services voor e-maillevering aanschaffen (Essentials, Enhanced of Plus). Dit staat los van elk leverbaarheidspakket op een reeds bestaand Marketo Engage-exemplaar. Leveringsservices worden per instantie gekoppeld, niet per organisatie. Voor ondersteuning van de leveringsmogelijkheden op beide instanties zijn twee aparte pakketten voor de leveringsservices vereist. Wanneer een nieuw IP voor Journey Optimizer B2B edition wordt voorzien, wordt een nieuw pakket van de Diensten van de Leverbaarheid vereist voor IP opwarming en aan de gang zijnde leveringssteun.

## DNS-records maken voor bestemmingspagina&#39;s en e-mail
...
Als beheerder, zou u met uw team van de Marketing moeten werken om twee verslagen van CNAME te plannen en uit te voeren. De eerste is voor het landen van pagina-URL&#39;s, zodat de bestemmingspagina&#39;s worden weergegeven in URL&#39;s die uw domein weerspiegelen en niet Adobe Marketo Engage (de werkelijke host). De tweede is voor de trackingkoppelingen die zijn opgenomen in de e-mails die via Marketo Engage worden verzonden.

### CNAME toevoegen voor bestemmingspagina&#39;s

Voeg de openingspagina CNAME toe aan uw DNS-record, zodat `[YourLandingPageCNAME]` verwijst naar de unieke accounttekenreeks die is toegewezen aan uw bestemmingspagina&#39;s. Meld u aan bij de site van uw domeinregistrar en voer de CNAME- en accounttekenreeks van de bestemmingspagina in. Dit item bestaat meestal uit drie velden:

* Alias: Enter `[YourLandingPageCNAME]`
* Type: CNAME
* Punt voor: Enter `[MunchkinID].mktoweb.com`
...
U kunt dezelfde DKIM-configuratie gebruiken voor uw productie-Marketo Engage-instantie en de bijgevoegde Journey Optimizer B2B edition-instantie. Maak in de bijgevoegde instantie hetzelfde exacte domein als in uw Marketo Engage-instantie. De waarden voor kiezer en codering hoeven niet overeen te komen. Nadat het domein aan de instantie van Journey Optimizer B2B edition wordt toegevoegd, open een de steunkaartje van Adobe om te verzoeken dat uw configuratie van DKIM van uw instantie van Marketo Engage aan de nieuwe instantie wordt gedeeld. Geef het Marketo Engage-voorvoegsel (Munchkin-id) en het nieuwe Journey Optimizer B2B edition-voorvoegsel (Munchkin-id) op.
...
* Een geldige SPF
* Een geldige DKIM-record voor uw FROM:-domein (aanbevolen voor [!DNL Marketo Engage] en [!UICONTROL Journey Optimizer B2B Edition])
...
**migrerend specifieke IPs aan Journey Optimizer B2B Edition**

Als u IP&#39;s hebt toegewezen, moet u de nieuwe Journey Optimizer B2B edition-instantie hebben gemaakt in hetzelfde gebied als uw bestaande Marketo Engage-instantie. Als de nieuwe instantie zich in een ander gebied bevindt, is het niet mogelijk om het bestaande IP te delen. Als het gebied aanpast, open een kaartje met [&#x200B; Steun van Adobe &#x200B;](https://experienceleague.adobe.com/home?lang=nl-NL&support-tab=home#support){target="_blank"} om te verzoeken dat uw bestaande IP en bindende groepen met de nieuwe instantie worden gedeeld. Geef het Marketo Engage-voorvoegsel (Munchkin-id) en het nieuwe Journey Optimizer B2B edition-voorvoegsel (Munchkin-id) op.

Met dit verzoek, repliceert Adobe zelfde IPs, bindt groepen, en gevormde terugkeer-weg domeinen zoals uw bestaande instantie van Marketo Engage. Wanneer IPs tussen uw Marketo Engage instantie en Journey Optimizer B2B edition wordt gedeeld, allebei gebruiken zij tezelfdertijd (verzendt van Marketo Engage en verzendt van Journey Optimizer B2B edition gebruikt de zelfde IPs).
...
De klanten op de gedeelde IP pool hebben geen extra configuratie nodig. U blijft de zelfde IP pools en het zelfde gebrek terugkeer-Weg domein gebruiken zoals voordien.
